...
// Config
let DISCORD_SERVER_URI: String = "https://discord.gg/9BJWSrEBBJ";
...
client.on(Events.ClientReady, () => {
	...
	client.user?.setActivity("AntiRaid Forums", { type: ActivityType.Watching });
});
				...
				if (command.data.permissionRequired) {
					...
					const row = new ActionRowBuilder().addComponents(
						new ButtonBuilder()
							...
							.setURL("https://antiraid.xyz/forums/profile/edit")
					);
					...
					await interaction.reply({
						embeds: [
							new EmbedBuilder()
								.setTitle("Oops!")
								.setDescription(
									`Sorry, you cannot use this command.\nReason: AntiRaid Forums account not linked with Discord.`
								)
								.setColor("Random"),
						],
